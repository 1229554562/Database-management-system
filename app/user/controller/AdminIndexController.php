...
class AdminIndexController extends AdminBaseController
{
    ...
    {
        $editid = input('param.id',0,'intval');
        if(request()->isPost()){
            ...
            if($res){
                $this->success('修改成功','/user/admin_index',['parent'=>1]);
            }else{
                $this->error('修改失败');
            }
        }
        $use = Db::name('user')
        ->field('id,mobile,user_login,user_name')
        ->where('id',$editid)
        ->find();
        $this->assign('use',$use);
    ...
    public function delete(){
        ...
        $res = Db::name('user')->where(id,$delid)->delete();
        if($res){
            $this->success('删除成功','/user/admin_index',['parent'=>1]);
        }else{
            $this->error('修改失败');
        }
    }
