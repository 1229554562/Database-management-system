;(function () {
     ...
     * 扩展生成appkey 的ajax请求
     */
    if ($('a.js-ajax-appkey').length){
        Wind.use('artDialog', 'noty3', function () {
            $('.js-ajax-appkey').on('click',function (e) {
                e.preventDefault();
                var $_this  = this,
                    $this   = $($_this),
                    msg = "您确定要进行此操作吗？",
 ...
 * @param app  应用名，CMF的应用名
 */
function openUploadDialogApi(dialog_title, callback, extra_params, multi, filetype, app) {
    Wind.css('artDialog');
    multi      = multi ? 1 : 0;
    filetype   = filetype ? filetype : 'image';
    app        = app ? app : GV.APP;
    var params = '&multi=' + multi + '&filetype=' + filetype + '&app=' + app;
    Wind.use("artDialog", "iframeTools", function () {
        art.dialog.open(GV.ROOT + 'user/Asset/webuploaderApi?' + params, {
            title: dialog_title,
            id: new Date().getTime(),
            width: '600px',
            height: '350px',
 ...
 * @param app  应用名,CMF的应用名
 */
function uploadOneImageApi(dialog_title, input_selector, extra_params, app) {
    openUploadDialogApi(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
        $(input_selector + '-preview').attr('src', files[0].preview_url);
        $(input_selector + '-name').val(files[0].name);
    }, extra_params, 0, 'image', app);
